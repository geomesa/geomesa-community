...
package org.geomesa.community.blob.handlers.exif
...
import org.geomesa.community.blob.api.handlers.AbstractFileHandler
...
class MetadataFileHandler extends AbstractFileHandler {
  ...
  override def canProcess(file: File, params: util.Map[String, String]): java.lang.Boolean = {
    val gps = ImageMetadataReader.readMetadata(file).getDirectoriesOfType(classOf[GpsDirectory])
    if (gps == null) {
      false
    } else {
